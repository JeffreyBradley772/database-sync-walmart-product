...
export class SyncController {
  ...
  @ApiOperation({
    ...
    description: 'Syncs products from Walmart API to the database',
  })
  @ApiResponse({ status: 200, description: 'Products successfully synced' })
  @ApiResponse({
    status: 500,
    description: 'Internal server error - Failed to sync products',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Fetches and syncs multiple pages of products from Walmart API to the database',
  })
  @ApiQuery({
    name: 'pages',
    required: false,
    type: Number,
    description: 'Number of pages to fetch (default: 200)',
  })
  @ApiResponse({ status: 200, description: 'Products successfully synced' })
  @ApiResponse({
    status: 500,
  ...
  async syncPaginatedProducts(@Query('pages') pages?: number): Promise<{
    success: boolean;
    totalPages: number;
    totalProducts: number;
    message: string;
  }> {
    return await this.syncService.syncWithPagination(
